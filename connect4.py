...
def is_valid_location(board, col):
    ...
    # if this is true, there is room in that column for a piece
...
def draw_board(board):
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            pygame.draw.rect(screen, NYANZA, (c*SQUARESIZE, r*SQUARESIZE+SQUARESIZE, SQUARESIZE, SQUARESIZE))
            pygame.draw.circle(screen, CIRCLEBLACK, (int(c*SQUARESIZE+SQUARESIZE/2), int(r*SQUARESIZE+SQUARESIZE+SQUARESIZE/2)), RADIUS)
...
myfont = pygame.font.SysFont("silom", 70)

while not game_over:

    for event in pygame.event.get():
        # allows game to properly exit out
        if event.type == pygame.QUIT:
            sys.exit()

        if event.type == pygame.MOUSEMOTION:
            pygame.draw.rect(screen, CIRCLEBLACK, (0,0,width, SQUARESIZE))
            posx = event.pos[0]
            if turn == 0:
